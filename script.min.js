...
// SPA Tab Pages Navigation System
function initTabSystem() {
    ...
    function switchPage(pageId) {
        ...
        if (pageId === 'why-choose') targetPageId = 'home';
        ...
        links.forEach(link => {
            ...
            if (href === `#${targetPageId}` || (targetPageId === 'home' && href === '#home')) {
    ...
    // Intercept clicks on links that reference pages
    document.addEventListener('click', function (e) {
        const link = e.target.closest('a, .cta-btn');
        if (!link) return;

        const href = link.getAttribute('href');
        if (href && href.startsWith('#')) {
            const targetId = href.substring(1);
            if (document.getElementById(`page-${targetId}`) || targetId === 'why-choose' || targetId === 'home' || targetId === 'features') {
                e.preventDefault();
                switchPage(targetId);
    ...
    if (hash && hash.startsWith('#')) {
        ...
        if (targetId === 'features') checkId = 'features';
        if (document.getElementById(`page-${checkId}`)) {
            initialPage = checkId;
        }
    }
